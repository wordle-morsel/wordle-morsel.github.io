...
function App() {
  ...
  const onEnter = () => {
    ...
    const currentGuessWord = currentGuess.trim()
    ...
    if (!isWordInWordList(currentGuessWord)) {
    ...
    // fill the rest of the squares with spaces
    var currentGuessPadded = currentGuess
    while (currentGuessPadded.length < MAX_WORD_LENGTH) {
      currentGuessPadded += ' '
    }
    setCurrentGuess(currentGuessPadded)
    ...
    const winningWord = isWinningWord(currentGuessWord)

    if (guesses.length < MAX_CHALLENGES && !isGameWon) {
      setGuesses([...guesses, currentGuessPadded])
      setCurrentGuess('')
